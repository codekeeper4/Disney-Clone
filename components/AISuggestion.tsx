...
function AISuggestion({term} :{term: string}) {
    const { data, error, isLoading, isValidating } = useSWR('suggestion', () => fetcher(term), {
    ...
    const generateText = () => {
        ...
        if(error) return <>AI Assistant is out playing golf at the moment...</>
        if(!data) return <>No data</>

        return(
            <>
                <div className='animate-pulse rounded-full bg-gradient-to-t from-white 
                                h-10 w-10 border-2 flex-shrink-0 border-white' 
                />
